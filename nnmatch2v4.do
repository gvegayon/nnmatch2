...
cap cd "C:\Users\Eduardo\Dropbox\Trabajo\publicaciones\proyecto nnmatch"

cap mata mata drop ls()
mata:
real colvector ls(
...
program def nnmatch2main, eclass sortpreserve
	...
	mata: st_store((1, `ntouse'), 
		...
		nn_match(
			...
			"`metric'",
			`m',
			&(`matches' = J(0,5+`nvars'*2,.)) // Pointer to matches matrix (used for computing biasadj)
			)
		);
	#delimit cr 
	...
	// Regression (MATA)
	mata: st_local("tau_est", strofreal(ls(st_data(.,"`touse2'"),st_data(.,"`tau'"),., 1)))
...
real matrix calc_distances(
	...
	// Computes distances
	for(i=1;i<=Ni;i++) {
			candidates = candidates\xdif[|i,.|]*V*transposeonly(xdif[|i,.|])
	}
	...
	)
{
	...
	betas0 = betas0[|1,1\1,kbias|]
	betas1 = betas1[|1,1\1,kbias|]
